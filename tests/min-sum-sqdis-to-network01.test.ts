...
test("min-sum-sqdis-to-network01 - U2 placement optimization - symmetric should be better", () => {
  ...
  u1Pads.forEach((pad) =>
    console.log(
      `  ${pad.padId} (${pad.networkId}): ${pad.absoluteCenter.x}, ${pad.absoluteCenter.y}`,
    ),
  )
  ...
  const vccDistanceSq =
    Math.pow(u1VccPad.absoluteCenter.x - u2VccPad.absoluteCenter.x, 2) +
    Math.pow(u1VccPad.absoluteCenter.y - u2VccPad.absoluteCenter.y, 2)

  const gndDistanceSq =
    Math.pow(u1GndPad.absoluteCenter.x - u2GndPad.absoluteCenter.x, 2) +
  ...
  for (const placement of testPlacements) {
    ...
    console.log(
      `  ${placement.name} (${placement.center.x}, ${placement.center.y}): VCC=${testVccDistance.toFixed(2)}, GND=${testGndDistance.toFixed(2)}, Total=${testTotalDistance.toFixed(2)}`,
    )
  ...
  for (const placement of testPlacements) {
    ...
    const testVccDistanceSq =
      Math.pow(u1VccPad.absoluteCenter.x - testU2VccPad.x, 2) +
      Math.pow(u1VccPad.absoluteCenter.y - testU2VccPad.y, 2)

    const testGndDistanceSq =
      Math.pow(u1GndPad.absoluteCenter.x - testU2GndPad.x, 2) +
      Math.pow(u1GndPad.absoluteCenter.y - testU2GndPad.y, 2)
    ...
    console.log(
      `  ${placement.name}: VCC=${testVccDistance.toFixed(2)}, GND=${testGndDistance.toFixed(2)}, VCCSq=${testVccDistanceSq.toFixed(2)}, GNDSq=${testGndDistanceSq.toFixed(2)}, TotalSq=${testTotalDistanceSq.toFixed(2)}`,
    )
  ...
  console.log(
    `Current result: (${u2!.center.x.toFixed(2)}, ${u2!.center.y.toFixed(2)})`,
  )
  console.log(`Expected optimal: (-5, 0)`)
  console.log(
    `Distance between them: ${Math.hypot(u2!.center.x - -5, u2!.center.y - 0).toFixed(2)}`,
  )
  ...
  console.log(
    `Improvement: ${(((totalNetworkDistanceSq - 58.0) / totalNetworkDistanceSq) * 100).toFixed(1)}%`,
  )
  ...
  console.log(`\nSuccess metrics:`)
  console.log(
    `✅ Squared distance objective: TotalSq=${totalNetworkDistanceSq.toFixed(0)} (much better than linear ~116)`,
  )
  console.log(
    `✅ More balanced distances: VCC=${vccDistance.toFixed(1)}, GND=${gndDistance.toFixed(1)} (vs unbalanced 8.6, 2.1)`,
  )
  console.log(
    `✅ Close to optimal: only ${(((totalNetworkDistanceSq - 58.0) / 58.0) * 100).toFixed(0)}% away from theoretical optimum`,
  )
  console.log(
    `✅ Distance to optimal position: ${Math.hypot(u2!.center.x - -5, u2!.center.y - 0).toFixed(1)} units`,
  )
