...
test("verify network03 C1 and C2 pad dimensions when rotated", () => {
  ...
  console.log(`C1: rotation=${c1.ccwRotationOffset}°, center=(${c1.center.x.toFixed(3)}, ${c1.center.y.toFixed(3)})`)
  console.log(`C2: rotation=${c2.ccwRotationOffset}°, center=(${c2.center.x.toFixed(3)}, ${c2.center.y.toFixed(3)})`)

  const c1Body = c1.pads.find(p => p.padId === "C1-body")!
  const c2Body = c2.pads.find(p => p.padId === "C2-body")!

  console.log(`\nPad dimensions:`)
  console.log(`  C1-body: ${c1Body.size.x.toFixed(3)} x ${c1Body.size.y.toFixed(3)}`)
  console.log(`  C2-body: ${c2Body.size.x.toFixed(3)} x ${c2Body.size.y.toFixed(3)}`)
  ...
  if (c1.ccwRotationOffset === 0 || c1.ccwRotationOffset === 180) {
    console.log(`  C1 (${c1.ccwRotationOffset}°): Should keep original aspect - narrow and tall: ${c1IsNarrowAndTall ? '✓' : '✗'}`)
    expect(c1IsNarrowAndTall).toBe(true)
  } else if (c1.ccwRotationOffset === 90 || c1.ccwRotationOffset === 270) {
    console.log(`  C1 (${c1.ccwRotationOffset}°): Should be wide and short: ${!c1IsNarrowAndTall ? '✓' : '✗'}`)
    expect(c1IsNarrowAndTall).toBe(false)
  }

  if (c2.ccwRotationOffset === 0 || c2.ccwRotationOffset === 180) {
    console.log(`  C2 (${c2.ccwRotationOffset}°): Should keep original aspect - narrow and tall: ${!c2IsWideAndShort ? '✓' : '✗'}`)
    expect(c2IsWideAndShort).toBe(false)
  } else if (c2.ccwRotationOffset === 90 || c2.ccwRotationOffset === 270) {
    console.log(`  C2 (${c2.ccwRotationOffset}°): Should be wide and short: ${c2IsWideAndShort ? '✓' : '✗'}`)
