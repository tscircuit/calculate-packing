...
test("debug C6 rotation selection in network02", () => {
  ...
  for (const pad of u1.pads) {
    if (pad.networkId.startsWith("C1.")) {
      console.log(`  ${pad.padId} (${pad.networkId}): (${pad.absoluteCenter.x.toFixed(3)}, ${pad.absoluteCenter.y.toFixed(3)})`)
    }
  }
  
  console.log(`\nC6 result:`)
  console.log(`  Rotation: ${c6.ccwRotationOffset}°`)
  console.log(`  Center: (${c6.center.x.toFixed(3)}, ${c6.center.y.toFixed(3)})`)
  ...
  console.log(`  Total squared: ${(c11_distance * c11_distance + c12_distance * c12_distance).toFixed(3)}`)
  ...
  for (const testRotation of [0, 90, 180, 270]) {
    ...
    for (const pad of c6_input.pads) {
      ...
      console.log(`  ${pad.padId}: distance to ${target_pad.padId} = ${distance.toFixed(3)}`)
    }
  }
