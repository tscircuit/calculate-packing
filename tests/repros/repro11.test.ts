...
type Orientation = "cw" | "ccw"

/**
 * Compute orientation ("cw" or "ccw") of a closed outline.
 *
 * Accepts either:
 *  - Array<Point>                        // list of vertices
 *  - Array<[Point, Point]>              // list of segments; uses the first point of each pair
 */
export function getOutlineOrientation(outline: Segment[]): Orientation {
  // Normalize to a simple list of vertices
  const vertices = outline.map(([p]) => p)
  const n = vertices.length
  if (n < 3) {
    // Degenerate: treat as cw by default
    return "cw"
  }
...
test("repro11", () => {
  ...
  console.log(getOutlineOrientation(solver.outlines[1]!))

  console.log(solver.outlines.map(getOutlineOrientation))
  expect(solver.outlines.map(getOutlineOrientation)).not.toContain("cw")
})
