...
test("pack soic8 with resistor and capacitor without overlap", () => {
  ...
  componentBounds.forEach((bounds, i) => {
    ...
    console.log(`${component.componentId}: center=(${component.center.x.toFixed(3)}, ${component.center.y.toFixed(3)}), rotation=${component.ccwRotationOffsetDegrees}°`)
    console.log(`  bounds: x=${bounds.x.toFixed(3)}, y=${bounds.y.toFixed(3)}, width=${bounds.width.toFixed(3)}, height=${bounds.height.toFixed(3)}`)
  })
