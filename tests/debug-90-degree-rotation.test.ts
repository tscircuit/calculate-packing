...
test("debug 90 degree rotation - should make horizontal pads vertical", () => {
  ...
  console.log(`  Left pad rotated to: (${rotatedLeft.x.toFixed(2)}, ${rotatedLeft.y.toFixed(2)})`)
  console.log(`  Right pad rotated to: (${rotatedRight.x.toFixed(2)}, ${rotatedRight.y.toFixed(2)})`)
  ...
  if (Math.abs(rotatedLeft.y - rotatedRight.y) > 9) {
    console.log(`✅ SUCCESS: Pads are vertically separated by ${Math.abs(rotatedLeft.y - rotatedRight.y)}`)
  } else {
...
test("test actual packing with 90 degree constraint", () => {
  ...
  console.log(`U2 center: (${u2.center.x.toFixed(1)}, ${u2.center.y.toFixed(1)})`)
  
  const pad1 = u2.pads[0]
  const pad2 = u2.pads[1]
  
  console.log(`Pad 1: (${pad1.absoluteCenter.x.toFixed(1)}, ${pad1.absoluteCenter.y.toFixed(1)})`)
  console.log(`Pad 2: (${pad2.absoluteCenter.x.toFixed(1)}, ${pad2.absoluteCenter.y.toFixed(1)})`)
  ...
  console.log(`Same X coordinate: ${sameX} (diff: ${Math.abs(pad1.absoluteCenter.x - pad2.absoluteCenter.x).toFixed(2)})`)
  console.log(`Different Y coordinate: ${differentY} (diff: ${Math.abs(pad1.absoluteCenter.y - pad2.absoluteCenter.y).toFixed(2)})`)
