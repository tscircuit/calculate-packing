...
test("simulate PackDebugger step behavior with 90° constraint", () => {
  ...
  packInput.components.forEach((comp, i) => {
    console.log(`  ${comp.componentId}: availableRotationDegrees = ${comp.availableRotationDegrees}`)
  })
  ...
  console.log(`\nAfter stepping:`)
  console.log(`U2 rotation: ${(u2Result.ccwRotationOffset * 180 / Math.PI).toFixed(1)}°`)
  console.log(`U2 center: (${u2Result.center.x.toFixed(1)}, ${u2Result.center.y.toFixed(1)})`)
  
  const pad1 = u2Result.pads[0]
  const pad2 = u2Result.pads[1]
  console.log(`U2 pads:`)
  console.log(`  ${pad1.padId}: (${pad1.absoluteCenter.x.toFixed(1)}, ${pad1.absoluteCenter.y.toFixed(1)})`)
  console.log(`  ${pad2.padId}: (${pad2.absoluteCenter.x.toFixed(1)}, ${pad2.absoluteCenter.y.toFixed(1)})`)
  
  // Check if U2 is vertical (same X, different Y by ~10 units)
  const isVertical = Math.abs(pad1.absoluteCenter.x - pad2.absoluteCenter.x) < 1 &&
  ...
  if (!rotation90) {
    console.log(`❌ BUG: U2 should be forced to 90° but got ${(u2Result.ccwRotationOffset * 180 / Math.PI).toFixed(1)}°`)
  }
