...
test("translation optimizer debug - should find optimal (-5, 0) position", () => {
  ...
  const bounds = computeTranslationBounds(u2Component, initialCenter, packedComponents, minGap)
  ...
  const withinBounds = optimalPoint.x >= bounds.minX && optimalPoint.x <= bounds.maxX &&
                       optimalPoint.y >= bounds.minY && optimalPoint.y <= bounds.maxY
  ...
  for (const testPoint of testPoints) {
    const cost = calculateSumDistance(u2Component, testPoint.point, packedComponents, true)
    console.log(`  ${testPoint.name} (${testPoint.point.x}, ${testPoint.point.y}): cost=${cost.toFixed(2)}`)
  }
  ...
  console.log(`  Geometric median: (${optimizedCenter.x.toFixed(2)}, ${optimizedCenter.y.toFixed(2)})`)
  console.log(`  Sampling: (${optimizedCenterSampling.x.toFixed(2)}, ${optimizedCenterSampling.y.toFixed(2)})`)
  console.log(`  Expected optimal: (-5, 0)`)

  const initialCost = calculateSumDistance(u2Component, initialCenter, packedComponents, true)
  const optimizedCost = calculateSumDistance(u2Component, optimizedCenter, packedComponents, true)
  const optimizedCostSampling = calculateSumDistance(u2Component, optimizedCenterSampling, packedComponents, true)
  const optimalCost = calculateSumDistance(u2Component, optimalPoint, packedComponents, true)
  ...
  const bestResult = optimizedCostSampling < optimizedCost ? optimizedCenterSampling : optimizedCenter
  ...
  console.log(`Distance to optimal: ${Math.hypot(bestResult.x - (-5), bestResult.y - 0).toFixed(2)}`)
  ...
  for (const point of gridPoints) {
    const cost = calculateSumDistance(u2Component, point, packedComponents, true)
    const hasOverlap = checkOverlap(u2Component, point, packedComponents, minGap)
    ...
    if (point.x === -5 && point.y === 0) {
      ...
      const u2Pads = [
        { center: { x: point.x - 5, y: point.y }, size: { x: 1, y: 1 }, label: "U2_VCC" }, // (-10, 0)
        { center: { x: point.x + 5, y: point.y }, size: { x: 1, y: 1 }, label: "U2_GND" }, // (0, 0)
      ]
      ...
      for (const u2Pad of u2Pads) {
        for (const u1Pad of u1Pads) {
          const dist = Math.hypot(u2Pad.center.x - u1Pad.center.x, u2Pad.center.y - u1Pad.center.y)
          const edgeToEdgeDist = dist - (Math.max(u2Pad.size.x, u2Pad.size.y) + Math.max(u1Pad.size.x, u1Pad.size.y)) / 2
          console.log(`    ${u2Pad.label} to ${u1Pad.label}: center-center=${dist.toFixed(2)}, edge-edge=${edgeToEdgeDist.toFixed(2)}`)
        }
      }
    }
    
    console.log(`  (${point.x}, ${point.y}): cost=${cost.toFixed(2)}, overlap=${hasOverlap}`)
  }
